...
module ActiveSupport
  ...
  class BufferedLogger
    ...
    def add(severity, message = nil, progname = nil, &block)
      ...
      #message = "\033[0;37m#{Time.now.to_s(:db)}\033[0m [\033[#{color}m" + sprintf("%-5s","#{sevstring}") + "\033[0m] #{message.strip}\n" unless message[-1] == ?\n
      message = "[\033[#{color}m" + sprintf("%-5s","#{sevstring}") + "\033[0m] #{message.strip}\n" unless message[-1] == ?\n
      buffer << message
      auto_flush
      message
    end
